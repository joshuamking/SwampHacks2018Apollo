...
class CameraUtilsBare {
    ...
    public CameraUtilsBare(CameraManager manager, File file, Supplier<Display> displaySupplier, Supplier<AutoFitTextureView> textureViewSupplier, Runnable captureCompleted) {
    ...
    private void setUpCameraOutputs(int width, int height) {
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                float ratio = ((float) largest.getHeight()) / 256;
                largest = new Size((int) (((float) largest.getWidth()) * ratio), 256);
